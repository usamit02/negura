...
export class CalendarModal implements OnInit, AfterViewInit {
  ...
  init(): void {
    ...
    this.calendarMonths = this.calSvc.createMonthsByPeriod(
      ...
      this.findInitMonthNumber(this._d.defaultScrollTo) + this.step, this._d
    );
  ...
  onChange(data: any): void {
    ...
    const from = new Date(data[0].time);
    let to = data[1]? new Date(data[1].time):null;
    let toStr;
    if(to){
      if(to.getTime() === from.getTime()){
        toStr = `日帰り`;
      }else{
        toStr = `～ ${to.getMonth()+1}月${to.getDate()}日（${days[to.getDay()]}）`;
      }
    }else{
      toStr = `～ チェックアウト日を指定してください`;
    }
    this.title =`${from.getMonth() + 1}月${from.getDate()}日（${days[from.getDay()]}）${toStr}`;
  }
