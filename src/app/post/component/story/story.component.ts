    ...
    let alart = await this.alert.create({
      ...
      buttons: [{ text: '取消', role: 'cancel' }, {
        text: '決定', handler: async (res) => {
          ...
          if (txt.indexOf("<iframe") > -1) {
            ...
            if (urls && urls.length) {
              ...
              if (widths && heights && widths.length && heights.length) {
                ...
                this.ui.alert("iframeのサイズを解析できませんでした。広告ですか");
              }
            } else {
              this.ui.alert("iframeを解析できませんでした。広告ですか"); return;
            }
          ...
          } else {//要改良　html生挿入は権限のあるアカウントにのみ認める方向で。
            await this.ui.confirm('html挿入', 'htmlを解析できませんでした。このまま挿入しますか？セキュリティリスクあり').catch(() => { return false; });
            html = txt;
            /*
