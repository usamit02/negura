...
export class MarkerComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (!this.markers.length) {
      ...
      this.markers = res.story_markers.map(marker => {
        marker.option = { position: { lat: marker.lat, lng: marker.lng }, icon: this.markericon[marker.icon] };
        return marker;
      })
    }
    ...
    if (markers.length) {
      ...
      const position = this.story.lat && this.story.lng ? { lat: this.story.lat, lng: this.story.lng } : this.center;
      ...
      marker = {
        id: this.story.id, na: na, txt: txt, img: "", icon: 1, lat: position.lat, lng: position.lng, idx: this.story.idx,
        option: { position: { lat: position.lat, lng: position.lng }, icon: this.markericon[1] }
      };
      this.markers.push(marker);
    }
    this.undo(marker);
  }
  async mapRightClick(e) {
  ...
  markerClick(marker: Marker, mapMarker: MapMarker) {
    this.openInfoWindow(marker, mapMarker);
    this.marker = marker;
  }
  markerRightClick(marker: Marker, mapMarker: MapMarker) {

  }
  openInfoWindow(marker: Marker, mapMarker: MapMarker) {
  ...
  undo(marker: Marker) {
    ...
    setTimeout(() => { this.center = { lat: marker.lat, lng: marker.lng } }, 3000);
        ...
        if (gpsLat && gpsLng) {
          ...
          if (!(this.marker.lat === lat && this.marker.lng === lng)) {
            if (await this.ui.confirm('マーカー位置', '写真に記録されたGPSデータに変更しますか。')) {
              ...
              this.marker.option = { position: { lat: lat, lng: lng }, icon: this.markericon[this.marker.icon] }
              this.markers = this.markers.map(marker => {
                if (marker.id === this.marker.id) {
                  marker.lat = lat; marker.lng = lng;
                  marker.option = { position: { lat: lat, lng: lng }, icon: this.markericon[marker.icon] }
                }
                return marker;
              })
            }
