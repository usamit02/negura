...
import { User, USER ,CHARGETYP } from '../../class';
...
export class ChargePage implements OnInit, OnDestroy {
  ...
  CHARGE = { typ: null, na: null, txt: "", ext: "", img: "", price: null, min: null, max: null, chat: 1, qty: null, size: null, close: 0 };
  charge = {
  ...
  constructor(private route: ActivatedRoute, private router: Router, private userService: UserService, private api: ApiService,
    ...
    private db: AngularFireDatabase, private store: AngularFirestore, private modal: ModalController,) { }
  ngOnInit() {
    this.route.params.pipe(takeUntil(this.onDestroy$)).subscribe(params => {
      ...
      if (params.id && params.room && params.prop) {
        this.userService.$.pipe(takeUntil(this.onDestroy$)).subscribe(async user => {
          if (user.id) {
            if (user.id !== this.user.id) {
              ...
              if (this.id) {
                this.api.get('query', { select: ['*'], table: 'charge', where: { prop: this.prop, id: this.id, room: { or: -1, or1: 0, or2: this.room } } }, "読込中").then(async res => {
        ...
        Object.keys(CHARGETYP).forEach(key => {
          this.unitTyps.push({ key: key, na: CHARGETYP[key].na });
        })
      }
    });
  }
  undo(method?: number) {
    let charge;
    if (method != null) {
      let charges = this.charges.filter(c => { return c.room === method; });
      if (charges.length) {
        charge = charges[0];
      } else {
        charge = this.CHARGE;
      ...
      charge = this.charges[0];
      this.method = this.charges[0].room;
    }
    ...
    if (this.editable) {
      if (this.chargeForm.dirty) {
        ...
        this.api.post('query', { table: "charge", duplicate: duplicate, insert: insert }).then(async () => {
          let charges = this.charges.filter(charge => { return charge.room === Number(this.method); });
          if (charges.length) {
            Object.keys(update).forEach(key => {
