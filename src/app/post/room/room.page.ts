...
export class RoomPage implements OnInit, OnDestroy {
  ...
  undo() {
    this.saving = true;
    this.api.get('query', { select: ['*'], table: 'room', where: { id: this.id } }).then(async res => {
      if (res.rooms.length !== 1) {
        this.id = null;
        throw { message: '無効なparam.idです。' };
      };
      this.roomForm.patchValue(res.rooms[0]);
        ...
        this.api.post("query", { table: "room", insert: insert }).then(async res => {
          if (copy && res.room) {
            ...
            if (calendar.room_calendars.length) {
              ...
              await this.api.post('querys', { table: "room_calendar", inserts: calendar.room_calendars });
            }
          }
          this.id = res.room.id;
          this.undo();
  ...
  async erase() {
    if (this.editable) {
      ...
      this.api.get('query', { table: 'story', select: ['file'], where: { typ: 'room', parent: this.id } }).then(async res => {
        ...
        this.id = null; this.roomForm.reset();
