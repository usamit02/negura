import { Component, OnInit, Input, ViewChild, ElementRef, OnDestroy, OnChanges, AfterViewInit, SimpleChanges, Renderer2 } from '@angular/core';
import { PopoverController, IonContent, DomController } from '@ionic/angular';
...
import { ScrollEndService } from 'src/app/service/scroll.service';
@Component({
  selector: 'app-send',
...
export class SendComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  @ViewChild('bar', { read: ElementRef, static: false }) bar: ElementRef;
  writer: string;
  message: string;
  ...
  constructor(private router: Router, private db: AngularFirestore, private api: ApiService, private storage: AngularFireStorage,
    private ui: UiService, private pop: PopoverController, private sendService: SendService, private scrollEnd: ScrollEndService,
    ...
    if (changes.user) {
      if (this.user.id) {
        ...
        setTimeout(() => {
          ...
          merge(paste$, change$, keyup$, keydown$).pipe(takeUntil(this.onDestroy$), throttleTime(2000)).subscribe(() => {
            this.sendable = true;
          });
      ...
      setTimeout(() => {//chatエリアにきたら投稿窓を表示
        this.height = this.bar.nativeElement.scrollHeight;//console.log("setHeight:",this.height);         
        this.scrollEnd.$.pipe(takeUntil(this.onDestroy$)).subscribe((state: any) => {
          if (this.chat != null) {
            const visible = state.y + state.h > this.chat.nativeElement.offsetTop ? true : false;
            const bar = this.bar.nativeElement;
            if (visible && !this.visible) {
              this.domCtrl.write(() => {
                this.renderer.setStyle(bar, 'opacity', `1`);
              });
              this.visible = true;
              setTimeout(() => { this.content.scrollByPoint(56, 0, 0); }, 500);
            } else if (!visible && this.visible) {
              this.domCtrl.write(() => {
                this.renderer.setStyle(bar, 'opacity', `0`);
              });
              this.visible = false;
            }
          }
  ...
  chatAdd(upd, media?, ext?) {
    ...
    let na = this.user.na ? this.user.na : "ななし";
    let avatar = this.user.avatar ? this.user.avatar : "";
    dbcon = this.db.collection(this.page).doc(this.id);
    if (this.thread) {
      dbcon = dbcon.collection("chat").doc(this.thread);
    }
    let add: any = { uid: uid, na: na, avatar: avatar, txt: txt, upd: upd };
    if (media) {
      add.media = media;
      add.ext = ext;
    } else if (this.media) {
