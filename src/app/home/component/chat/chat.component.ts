import { Component, AfterViewInit, Input, Output, EventEmitter, ViewChild, ViewChildren, QueryList, ElementRef, OnChanges, SimpleChanges, OnDestroy } from '@angular/core';
...
export class ChatComponent implements AfterViewInit, OnChanges, OnDestroy {
  @Input() param;
  @Input() page: string;
  @Input() user: User;
  ...
  constructor(private actionSheetCtrl: ActionSheetController, private route: ActivatedRoute, private router: Router, private readonly db: AngularFirestore
    , private storage: AngularFireStorage, private ui: UiService, private pop: PopoverController, private scrollEnd: ScrollEndService,) { }
  ngAfterViewInit() {
    setTimeout(() => {
      this.scrollEnd.$.pipe(takeUntil(this.onDestroy$)).subscribe(state => {
        this.Y = state.y - this.chatY.nativeElement.offsetTop;
        this.H = state.h;
        this.latest = state.H - this.Y - this.H > 300 ? true : false;
        if (this.Y + this.H > 0) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.param) {
      this.id = this.param.id ?.toString();
      this.cursor = this.param.cursor ? new Date(Number(this.param.cursor) * 1000) : null;
      this.thread = this.param.thread ? this.param.thread : "";
      this.topInfinite = this.param.topInfinite ? this.param.topInfinite : false;
    }
    if (this.user && this.id) this.init();
  }
  init() {
