...
export class ColumnsPage implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute, private state: StateService, private api: ApiService, private user: UserService, ) { }

  ngOnInit() {
    this.state.select(state => state.columns).pipe(takeUntil(this.onDestroy$)).subscribe(columns => {
      if (columns.length > 0) {
