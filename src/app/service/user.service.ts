...
export class UserService {
  ...
  constructor(private state: StateService, private fireAuth: AngularFireAuth, private db: AngularFireDatabase, private platform: Platform,) { }
  get(): User {
    ...
    return this.fireAuth.user.subscribe(async user => {
      if (user && user.uid) {
        if ((await this.db.database.ref(`ban/${user.uid}`).once('value')).val()) {
          ...
          if (dbuser) {
            data.na = dbuser.na ? dbuser.na : data.displayName;
            data.avatar = dbuser.avatar ? dbuser.avatar : data.photoURL;
            data.image = dbuser.image ? dbuser.image : data.photoURL;
            data.direct = dbuser.direct ? dbuser.direct : "";
          ...
          this.state.update(state => ({ ...state, user: data }));
        }
      } else {
